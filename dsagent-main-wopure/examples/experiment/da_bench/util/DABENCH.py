...
DABENCH_PATH = os.path.join(DATA_PATH, "di_dataset/da_bench")
...
async def ask(question: str, system_prompt: str) -> str:
    ...
    gpt4o_mini_reformat_config_path = METAGPT_ROOT / "config" / "reformat.yaml"
    gpt4o_mini_reformat_config = Config.from_yaml_file(gpt4o_mini_reformat_config_path)

    llm = LLM(llm_config=gpt4o_mini_reformat_config.llm)
...
class DABench:
    ...
    def generate_formatted_prompt(self, question_id: int) -> str:
        """
    ...
    def get_task_level(self, question_id: str) -> str:
    ...
    def evaluate_prediction(self, true_label, pred_dict):
        """
        Evaluate the prediction against the true label for all metrics.

        Args:
            true_label (list): A list of tuples, where each tuple is (metric, true_value).
            pred_dict (dict): The predicted values for each metric.
        ...
        for metric, true_value in true_label:
            ...
            if metric in pred_dict:
                # Consider the prediction correct if it's within a small tolerance for numeric values
                if isinstance(true_value, (int, float)) and isinstance(pred_dict[metric], (int, float)) and abs(
                        pred_dict[metric] - true_value) < 1e-6:
                    correctness[metric] = True  # Mark as correct if within tolerance

                # For string comparison, consider case-insensitive equality
                elif isinstance(true_value, str) and str(pred_dict[metric]).lower() == str(true_value).lower():
    ...
    def single_eval(self, id: str, prediction: str, use_reformat=False) -> dict:
        """
        ...
        prediction = prediction.replace("{", "").replace("}", "").replace("'", "").replace("```", "")  # Clean the prediction string
        ...
        correctness = self.evaluate_prediction(true_label, pred_dict)

        if not all(correctness.values()) and use_reformat:
            logger.warning(f"Not all metrics are correct: {correctness}")
            nest_asyncio.apply()
            prediction = self.async_reformat_prediction(id, prediction)
            pred_dict = parse_prediction(prediction)

            # Reevaluate after reformatting
            correctness = self.evaluate_prediction(true_label, pred_dict)

        return correctness  # Return the correctness dictionary

    def eval_all(self, id_list: list, predictions: list, use_reformat=False) -> dict:
        """
        ...
        for id, prediction in zip(id_list, predictions):
            correct = self.single_eval(id, prediction, use_reformat=use_reformat)
        ...
        return {
            ...
            "count": len(results),
            "sub_count": sum(len(result["correctness"]) for result in results if "correctness" in result),
            "results": results,
        }
