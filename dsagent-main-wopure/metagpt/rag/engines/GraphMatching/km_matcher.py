...
class KMMatcher:
    def __init__(self, weights):
        # desc - weights : n * m weight matrix (numpy , float), n <= m
        ...
        self.max_match = 0
        self.xy = -np.ones((self.n,), dtype=np.int64)
        self.yx = -np.ones((self.m,), dtype=np.int64)
    ...
    def find_augment_path(self):
        ...
        self.slackyx = -np.ones((self.m,), dtype=np.int64)

        self.prev = -np.ones((self.n,), np.int64)
