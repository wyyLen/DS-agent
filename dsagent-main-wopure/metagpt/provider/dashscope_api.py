...
def build_api_arequest(
    ...
    # 处理不同版本的dashscope返回值数量差异
    protocol_params = _get_protocol_params(kwargs)
    if len(protocol_params) == 11:
        # 旧版本返回11个值
        (
            api_protocol,
            ws_stream_mode,
        ...
        raise ValueError(f"Unexpected number of protocol params: {len(protocol_params)}")
