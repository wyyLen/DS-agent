...
def get_rag_engine_llm(model_infer: BaseLLM = None) -> RAGLLM:
    ...
    if model_infer is None:
        # Should never happen - always pass model_infer to avoid config validation
        raise ValueError("model_infer must be provided to avoid LLM config validation issues")
    llm = model_infer
...
class DSAgentStream(Role):
    ...
    def set_plan_and_tool(self):
        ...
        if self.use_rag:
            ...
            # 使用FAISS检索器
            from metagpt.rag.schema import FAISSRetrieverConfig
            # Use self.llm (already created by Role) instead of creating new LLM
            self.rag_engine = CustomEngine.from_docs(
                ...
                ranker_configs=[],
                llm=get_rag_engine_llm(model_infer=self.llm),
            )
        # 调整相似度阈值
        self.rag_exp_similarity_threshold = 0.33
        if self.tools:
