...
from dsagent_core.actions.ds_agent.ask_review import ReviewConst
from dsagent_core.actions.ds_agent.conclude_res import Conclusion
from dsagent_core.actions.ds_agent.extract_thought import ThoughtExtract
from dsagent_core.actions.ds_agent.query_utils import QueryUtils
from dsagent_core.actions.ds_agent.retrieval_exp import RetrievalExp, GenerateQuery, AdjustPlanFromWorkflow
from dsagent_core.actions.ds_agent.write_ds_code import WriteDsCode, CheckData
from dsagent_core.actions.ds_agent.write_ds_plan import RefinePlan
from metagpt.config2 import Config
from metagpt.const import METAGPT_ROOT
from dsagent_core.const import EXP_PLAN, WORKFLOW_EXP, EXAMPLE_DATA_PATH, DA_EVAL_RES_PATH
from metagpt.llm import LLM
from metagpt.logs import logger
from metagpt.memory import Memory
from dsagent_core.prompts.ds_agent.write_ds_code import DATA_INFO
...
from dsagent_core.rag.schema import MixtureRetrieverConfig
from metagpt.roles import Role
from metagpt.schema import Message, Task, TaskResult, Plan
from dsagent_core.strategy import ds_planner
from dsagent_core.strategy.ds_task_type import TaskType
from dsagent_core.strategy.lats_react import LanguageAgentTreeSearch, Node, collect_trajectory
...
def get_rag_engine_llm(model_infer: BaseLLM = None) -> RAGLLM:
    return RAGLLM(model_infer=model_infer or LLM(gpt4o_mini_config.llm))
...
class DSAgent(Role):
    ...
    def set_plan_and_tool(self):
        ...
        if isinstance(self.rag_engine, CustomMixtureEngine):
            self.rag_exp_similarity_threshold = 0.4  # (1 - 0.33) * 0.6 = 0.402
        if self.tools:
    ...
    async def _plan_and_act(self) -> Message:
        try:
            ...
            if use_lats:
                ...
                lats_res, best_child = await lats.enhance_run(iterations=10)
                conclusion = Message(content=lats_res, role="assistant")
    ...
    async def _write_and_exec_code(self, max_retry: int = 5, max_replanning_times: int = 2):
    ...
    def save_failed_cases(self):
        failed_cases_path = DA_EVAL_RES_PATH / "failed_cases/our_all_cases_4o_mini.jsonl"
