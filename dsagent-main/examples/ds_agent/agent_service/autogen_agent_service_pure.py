...
try:
    ...
    # If not in env, try loading from config2.yaml
    if not api_key:
        import yaml
        config_path = Path(__file__).parent.parent.parent.parent / 'config' / 'config2.yaml'
        ...
        if config_path.exists():
            with open(config_path, 'r', encoding='utf-8') as f:
                ...
                if api_key:
                    ...
                    logger.info(f"✅ Loaded API key from config file: {api_key[:10]}...")
        else:
    ...
    if api_key:
        ...
        logger.warning("⚠️ No API key found. Please set DASHSCOPE_API_KEY environment variable.")
...
class PureAutoGenDSAgent:
    ...
    def __init__(
        ...
        # Use independent executor (no MetaGPT dependency)
        from dsagent_core.actions import IndependentCodeExecutor
        self.code_executor = IndependentCodeExecutor(workspace_path=Path(self.work_dir))
        logger.info(f"🔧 Shared code executor initialized (independent)")
    ...
    def _get_default_llm_config(self) -> Dict:
        ...
        logger.info(f"Getting default LLM config, DASHSCOPE_API_KEY={'set' if api_key else 'not set'}")
        if not api_key:
